...
function create_db() {
    ...
    db.prepare('CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL, mail TEXT NOT NULL, password TEXT NOT NULL)').run()
    ...
    let addData = () => {
        db.prepare(`INSERT INTO users (username, mail, password) VALUES ('yanis','yanis@yanis.com' , 'kawkaw2003')`).run()
        db.prepare(`INSERT INTO restaurants (name, adress, type, budget) VALUES ('otacos', '605 avenue du prado, 13008, marseille', 'fast-food', '€' )`).run()
    }
    addData()
}
